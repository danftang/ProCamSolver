...
template<int V>
template<int F>
double MotionMatrix<V>::levmar_solve(const MeasurementMatrix<V> &measurement,
				     const ImageTransform<F> &fixedIntrinsics){
  int			info;
  CameraParams<V,F>	params(*this, fixedIntrinsics);
  NumericalDiff<CameraParams<V,F> >		   	 numDiff(params);
  LevenbergMarquardt<NumericalDiff<CameraParams<V,F> > > levMar(numDiff);
...
double MotionMatrix<V>::levmar_solve(const MeasurementMatrix<V> &measurement) {
  ...
  return(levmar_solve(measurement, dummy));
}
...
Eigen::Matrix4d MotionMatrix<V>::diac_euclidean_lift(int nCameras, const ImageTransform<F> &fixedIntrinsics) {
  ...
  // ---      [ 0  |1]
  // -----------------------------------------
  Eigen::Matrix3d	K;   // intrinsics of view 0
  ...
  Rot.fill(0.0);
  Rot.topLeftCorner(3,3) = ((K.inverse()*view(0)*H).leftCols(3)).inverse();
  Rot(3,3) = 1.0;
...
double MotionMatrix<VIEWS>::KR_decompose(ImageTransform<VIEWS> &K,
					 ImageTransform<VIEWS> &R) {
  int 			i;
  Eigen::Matrix3d 	MMT;		// M*M^T
  Eigen::Matrix3d 	K1;		// inverse intrinsics
  double		total_skew;

  // --- first calculate K from M*M^T
  // --- by Cholesky decomposition
  // --------------------------------
  total_skew = 0.0;
  for(i = 0; i<VIEWS; ++i) {
    // --- (MMT)^-1 = M^-TM^-1 which is in LL^T form
    MMT = M(i) * M(i).transpose();
    K1 = MMT.inverse().llt().matrixU();
    K.view(i) = K1.inverse();
    K.view(i).matrix() *= 1.0/K.view(i)(2,2);
    total_skew += fabs(K.view(i)(0,1));
  }

  // --- Now calculate R by multiplying
  // --- by  K^-1
  // ----------------------------------
  for(i=0; i<VIEWS; ++i) {
    R.view(i) = K.view(i).inverse() * M(i);
  }
  return(total_skew);
}
...
//////////////////////////////////////////////////////////////////////////////
template<int V>
MotionMatrix<V> &MotionMatrix<V>::reprojection_err(
				  const MeasurementMatrix<V> &E,
				  Eigen::VectorXd &err) {
  int 				i,j,n;
  Eigen::Matrix<double, 3*V, 1>	projectionCol;
  ShapeMatrix 			shape;
  ...
  n = 0;
  for(j=0; j<E.cols(); ++j) {
    err.middleRows<> = *this * shape.col(j);
    for(i = 0; )
    err(n) = 
  }
}
